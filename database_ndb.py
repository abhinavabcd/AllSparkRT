...
class Db():
    ...
    def is_server_node(self, node_id):
        ...
        return node and node.address != None

    def add_connection(self, node_id1, node_id2):
        connection_id = util_funcs.get_random_id(10)
        # TODO: check if already exists
        conn = ConnectionEntity(id=connection_id, from_node_key = ndb.Key('NodeEntity', node_id1),
    ...
    def create_node(self, client_id, addr, addr_internal, port):
        ...
        node = NodeEntity(id=node_id, client_id=client_id, address = addr, address_internal=addr_internal, port=port)
        node.put()
        return node_id
    """
    ...
    def node_config_exists(self, addr, port):
        query = NodeEntity.query(NodeEntity.address==addr, NodeEntity.port==port)
        node = query.get()
        if (node):
            return node.key.id()
        return None
    ...
    def create_session(self, name, description, client_id):
        ...
        self.sessions.insert_one(
            {"session_id": session_id, "name": name, "description": description, "client_id": client_id})
        session = SessionEntity(id=session_id, name=name, description=description, client_id=client_id)
        session.put()
        return session_id
